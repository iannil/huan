...
> 最后更新：2026-06-26（Stage 4 admin panel 规划落地） · 分支：master · 当前版本：**v0.2.3**
> 本文档替代 `docs/technical-plan.md` 第 8.4 节"剩余差异"——后者作为冻结的设计参考，**实际最新状态以此文档为准**。

## v0.2.x 系列（2026-06-13 夜 ~ 今）

v0.1.0 发版后连续三个版本，详见 [ADR 0005](../adr/0005-remove-encrypt-and-v02-feature-batch.md) 与 [`memory/daily/2026-06-13-v02.md`](../../memory/daily/2026-06-13-v02.md)。
...
**CLI 子命令当前总数**：13（build/serve/deploy/plugin/release/version/env/config/list/new/sync/toc/export）。

**`internal/` 包当前总数**：17（v0.1.0 时 18，v0.2.0 移除 encrypt 后 17）。
...
| 第一个 PR | 前后端一起（CRUD Go API + SPA 骨架） |
...
| PR | 范围 | 内容 |
|----|------|------|
| 1 | `internal/admin/` + `web/admin/` | Go API（内容列表/读取/写入/新建/删除）+ React SPA 骨架（路由+布局+Shadcn 主题）+ `//go:embed` 挂载到 `/admin` |
| | `ContentList 完整化（追加）` | 搜索、排序、草稿筛选、分页、内容类型标签、批量选择/删除/切换草稿（纯前端，2026-06-26） |

### CLI / API 变更（待定）

- `huan serve` 新增 `/admin/*` 路由
- `GET /api/admin/content` — 内容列表
- `GET /api/admin/content/{path}` — 单文件读取
- `POST /api/admin/content` — 新建
- `PUT /api/admin/content/{path}` — 更新
- `DELETE /api/admin/content/{path}` — 删除
