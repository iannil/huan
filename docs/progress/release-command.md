...
> 状态：进行中 · 起始：2026-06-13 · 负责人：iannil
...
- [ ] **Step 1**：`chore(license): add MIT LICENSE` — 添加 LICENSE 文件（release fail-fast 前提）
- [ ] **Step 2**：`refactor(observability): extract Logger from deploy` — `internal/deploy/logging.go` 整体迁入 `internal/observability/logging.go`，纯机械移动
- [ ] **Step 3**：`refactor(deploy): use observability.Logger` — deploy 包改 import，类型重命名 `deploy.Logger` → `observability.Logger`
- [ ] **Step 4**：`feat(version): expose VCS info via debug.ReadBuildInfo` — `internal/version/version.go` 加 `VCS() (sha, dirty, commitTime)`，`huan version` 输出 shortSHA
- [ ] **Step 5**：`feat(release): cross-compile + archive + checksums + manifest`（含单元测试）
...
- [ ] `go test ./...` 全通过（含 observability 重构后 deploy 测试不退化）
- [ ] `go test -tags=integration ./internal/release/...` 通过
- [ ] `go run ./cmd/huan release --dry-run --targets=current` 输出合法 manifest JSON 到 stdout，不写 /release
- [ ] `go run ./cmd/huan release` 在 `/release/0.1.0/` 产出 5 tarball/zip + checksums.txt + manifest.json
- [ ] `shasum -a 256 -c /release/0.1.0/huan_0.1.0_checksums.txt` 验证全过
- [ ] `tar tzf /release/0.1.0/huan_0.1.0_darwin_arm64.tar.gz` 列出 `huan` / `LICENSE` / `README.md` / `README.zh-CN.md` 四文件
- [ ] 解开 `darwin_arm64` tarball 后 `./huan version` 输出 `huan 0.1.0 (87b2836)` 或类似
- [ ] LICENSE 缺失时 `go run ./cmd/huan release` fail-fast 报错且不写任何文件
- [ ] VERSION 改为 `v0.1.0` 时 release 报"leading v not allowed"
- [ ] 两次连续 `go run ./cmd/huan release` 产出的二进制 sha256 字节一致

## 进展日志

（按时间倒序，每次会话追加。格式：YYYY-MM-DD HH:MM — 动作 + 结果。）

- 2026-06-13 — Step 1 完成：添加 MIT LICENSE（commit pending）
