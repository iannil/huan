...
> **当前实际结构**（v0.3.0）。`internal/encrypt/` 与 `pkg/` 已不存在：前者 v0.2.0 移除（[ADR 0005](adr/0005-remove-encrypt-and-v02-feature-batch.md)），后者从未建（YAGNI）。当前新增 `internal/admin/` + `internal/translate/` + `web/admin/`。

```
huan/
├── cmd/
│   ├── huan/              # CLI 入口（14 子命令，详见 docs/INDEX.md）
