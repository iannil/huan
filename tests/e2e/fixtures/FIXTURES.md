...
1. **`$HUAN_HOME`（默认 `~/.huan/plugins`）的陈旧 .so 与跨树构建失配**：loader 先扫 `$HUAN_HOME` 再扫项目 `plugins/`；`~/.huan` 里的 .so 若与当前 host 二进制非同一次构建（Go plugin 需同源码树、同 `pkg/` buildid），`plugin.Open`/`InitPlugin` 可能失败并仅 stderr warning、构建退出码仍 0 → 无注入。**2026-08-15 三场景对照实测更正**：【原「毒化连带拒绝」表述有误】——坏 .so（mach-o 损坏）或版本失配 .so 加载失败后，`ScanAndLoad` 的 `continue` 会正确放行项目同名 .so（同一源树的干净 .so 仍注入，og: ×4 实测），不存在「同名第二次 Open 连带失败」；Task 3 当时观察到的「静默零注入」实为 worktree 二进制 vs 主仓编译 .so 的 **buildid 跨树失配**（Go plugin 固有：插件必须与宿主同源码树构建）。**对策不变**：E2E 启动 daemon/build 前 `export HUAN_HOME=<空目录>` 隔离全局插件目录 + patterns.md §7 同树现场编译（该场景即环境契约，非引擎 bug；fix-e2e-found-bugs Task 6 定性记录）。
